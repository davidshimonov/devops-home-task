...
_TLoader = t.Callable[[t.Optional[str]],
                      tuple[t.Optional[str], t.Optional[_TOpener]]]
...
class SharedDataMiddleware:
    ...
    def _opener(self, filename: str) -> _TOpener:
        return lambda: (
            open(
                filename, "rb"), datetime.fromtimestamp(
                os.path.getmtime(filename), tz=timezone.utc), int(
    ...
    def get_package_loader(self, package: str, package_path: str) -> _TLoader:
        ...
        reader = spec.loader.get_resource_reader(
            package)  # type: ignore[union-attr]
    ...
    def __call__(
        ...
        mime_type = get_content_type(
            guessed_type[0] or self.fallback_mimetype, "utf-8")
        ...
        if self.cache:
            ...
            etag = self.generate_etag(
                mtime, file_size, real_filename)  # type: ignore
            headers += [
                ("Etag", f'"{etag}"'),
