    ...
    def read(self, *args: t.Any) -> bytes:
        if len(args) == 0:
            warn(
                ...
                " return from this call.", WSGIWarning, stacklevel=2, )
        elif len(args) != 1:
            warn(
        ...
        if self.headers_set:
            ...
            if status_code == 304:
                for key, _value in headers:
                    ...
                    if key not in (
                        "expires",
                            "content-location") and is_entity_header(key):
                        warn(
...
class LintMiddleware:
    ...
    def check_environ(self, environ: WSGIEnvironment) -> None:
        ...
        if environ["wsgi.version"] != (1, 0):
            warn(
                "Environ is not a WSGI 1.0 environ.",
                WSGIWarning,
                stacklevel=3)
    ...
    def check_start_response(
        ...
        for item in headers:
            if type(item) is not tuple or len(item) != 2:
                warn(
                    "Header items must be 2-item tuples.",
                    WSGIWarning,
                    stacklevel=3)
            name, value = item
    ...
    def check_iterator(self, app_iter: t.Iterable[bytes]) -> None:
        if isinstance(app_iter, str):
            warn(
                ...
                " Return a list or iterable instead.", WSGIWarning, stacklevel=3, )
    ...
    def __call__(self, *args: t.Any, **kwargs: t.Any) -> t.Iterable[bytes]:
        ...
        def checking_start_response(
            *args: t.Any, **kwargs: t.Any
            ...
            headers_set[:] = self.check_start_response(
