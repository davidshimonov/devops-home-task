...
def _find_common_roots(paths: t.Iterable[str]) -> t.Iterable[str]:
    ...
    for chunks in sorted(
        (PurePath(x).parts for x in paths),
        key=len,
            reverse=True):
        node = root
    ...
    def _walk(node: t.Mapping[str, dict[str, t.Any]],
              path: tuple[str, ...]) -> None:
...
def _get_args_for_reloading() -> list[str]:
    ...
    if getattr(__main__, "__package__", None) is None or (
        ...
        if os.path.isfile(py_script):
            ...
            # Incorrectly rewritten by pydevd debugger from "-m script" to
            # "script".
            py_module = py_script
...
class WatchdogReloaderLoop(ReloaderLoop):
    ...
    def run_step(self) -> None:
        ...
        for path in _find_watchdog_paths(
