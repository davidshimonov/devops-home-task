...
class ImmutableTypeConversionDict(
        ImmutableDictMixin[K, V], TypeConversionDict[K, V]):  # type: ignore[misc]
...
class MultiDict(TypeConversionDict[K, V]):
    ...
    def setlistdefault(
        ...
        if key not in self:
            ...
            super().__setitem__(key, list(default_list or ()))

        return super().__getitem__(key)  # type: ignore[return-value]

    # type: ignore[override]
    def items(self, multi: bool = False) -> cabc.Iterable[tuple[K, V]]:
...
class _OrderedMultiDict(MultiDict[K, V]):
    ...
    def __getitem__(self, key: K) -> V:
        if key in self:
            # type: ignore[index,no-any-return]
    ...
    def add(self, key: K, value: V) -> None:
        dict.setdefault(
            ...
            _omd_bucket(
                self,
                key,
                value))  # type: ignore[arg-type,attr-defined]
    ...
    def setlistdefault(
            self,
            key: t.Any,
            default_list: t.Any = None) -> t.NoReturn:
        raise TypeError(
    ...
    def poplist(self, key: K) -> list[V]:
        buckets: cabc.Iterable[_omd_bucket[K, V]] = dict.pop(
...
class CombinedMultiDict(
        ImmutableMultiDictMixin[K, V], MultiDict[K, V]):  # type: ignore[misc]
    ...
    def __init__(
            self, dicts: cabc.Iterable[MultiDict[K, V]] | None = None) -> None:
        super().__init__()
    ...
    @t.overload
    def items(self,
              multi: t.Literal[True]) -> cabc.Iterable[tuple[K,
                                                             list[V]]]: ...
...
class ImmutableDict(
        ImmutableDictMixin[K, V], dict[K, V]):  # type: ignore[misc]
