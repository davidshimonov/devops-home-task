...
def dump_options_header(
        header: str | None, options: t.Mapping[str, t.Any]) -> str:
...
def parse_dict_header(value: str) -> dict[str, str | None]:
    ...
    for item in parse_list_header(value):
        ...
        if key[-1] == "*":
            ...
            # adapted from parse_options_header, without the continuation
            # handling
            key = key[:-1]
...
def parse_options_header(value: str | None) -> tuple[str, dict[str, str]]:
    ...
    for pk, pv in parts:
        if pk[-1] == "*":
            # key*=charset''value becomes key=value, where value is percent
            # encoded
            pk = pk[:-1]
            match = _charset_value_re.match(pv)

            if match:
                # If there is a valid charset marker in the value, split it
                # off.
        ...
        # Remove quotes. At this point the value cannot be empty or a single
        # quote.
        if pv[0] == pv[-1] == '"':
            # HTTP headers use slash, multipart form data uses percent
            pv = pv[1:-1].replace("\\\\", "\\").replace('\\"',
                                                        '"').replace("%22", '"')
...
def parse_accept_header(
    value: str | None,
    cls: type[_TAnyAccept]) -> _TAnyAccept: ...
...
def remove_hop_by_hop_headers(
        headers: ds.Headers | list[tuple[str, str]]) -> None:
...
def dump_cookie(
    ...
    if domain:
        domain = domain.partition(":")[0].lstrip(
    ...
    # three octal digits, which matches http.cookies, although the RFC
    # suggests base64.
    if not _cookie_no_quote_re.fullmatch(value):
        # Work with bytes here, since a UTF-8 character could be multiple
        # bytes.
    ...
    if max_size and cookie_size > max_size:
        ...
        warnings.warn(
            ...
            " silently ignore cookies larger than this.", stacklevel=2, )
