...
class EnvironBuilder:
    ...
    def _get_form(
            self,
            name: str,
            storage: type[_TAnyMultiDict]) -> _TAnyMultiDict:
        ...
        if to_name_parts != from_name_parts:
            if to_name_parts[-len(from_name_parts):] == from_name_parts:
                if not self.allow_subdomain_redirects:
                    raise RuntimeError(
                        "Following subdomain redirects is not enabled.")
            else:
                raise RuntimeError(
...
class Cookie:
    ...
    def _from_response_header(
        ...
        decoded_key, decoded_value = next(
            parse_cookie(header).items())  # type: ignore[call-overload]
        params = {}
        ...
        return cls(
            ...
            expires=parse_date(
                params.get("expires")),
            max_age=int(
                params["max-age"] or 0) if "max-age" in params else None,
