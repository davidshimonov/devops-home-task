...
def do_urlencode(
    ...
    return "&".join(
        f"{url_quote(k, for_qs=True)}={url_quote(v, for_qs=True)}" for k,
        v in items)
...
def do_filesizeformat(
        value: t.Union[str, float, int], binary: bool = False) -> str:
...
def sync_do_groupby(
    ...
    if not case_sensitive:
        # Return the real key from the first value instead of the lowercase
        # key.
        output_expr = make_attrgetter(environment, attribute, default=default)
        out = [_GroupTuple(output_expr(values[0]), values)
               for _, values in out]
...
def do_attr(
    ...
    # Determine if the attr exists first, so we know the fallback won't
    # trigger.
    try:
        ...
        # This finds dynamic attrs, and we know it's not a descriptor at this
        # point.
...
def prepare_map(
    ...
    if not args and "attribute" in kwargs:
        ...
        try:
            ...
            raise FilterArgumentError(
                "map requires a filter argument") from None
...
def prepare_select_or_reject(
    ...
    if lookup_attr:
        try:
            ...
            raise FilterArgumentError(
                "Missing parameter for attribute name") from None
