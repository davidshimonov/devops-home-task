...
def _make_binop(
        op: str) -> t.Callable[["CodeGenerator", nodes.BinExpr, "Frame"], None]:
    @optimizeconst
    def visitor(
            self: "CodeGenerator",
            node: nodes.BinExpr,
...
def has_safe_repr(value: t.Any) -> bool:
    ...
    if type(value) is dict:  # noqa E721
        return all(has_safe_repr(k) and has_safe_repr(v)
...
class Frame:
    ...
    def __init__(
        ...
        # but they still needs to be kept track of as part of the active
        # context.
...
class CodeGenerator(NodeVisitor):
    ...
    def start_write(self, frame: Frame,
    ...
    def visit_Template(
        ...
        if have_extends:
            ...
            if not self.environment.is_async:
                ...
                self.writeline(
                    "agen = parent_template.root_render_func(context)")
                self.writeline("try:")
                self.indent()
    ...
    def visit_Block(self, node: nodes.Block, frame: Frame) -> None:
        ...
        if not self.environment.is_async and frame.buffer is None:
            ...
            self.writeline(
                f"gen = context.blocks[{node.name!r}][0]({context})")
            self.writeline("try:")
            self.indent()
            self.writeline(
    ...
    def visit_FromImport(self, node: nodes.FromImport, frame: Frame) -> None:
        ...
        for name in node.names:
            ...
            position = self.position(node).replace(
                "{", "{{").replace("}", "}}")
            message = (
    ...
    def visit_For(self, node: nodes.For, frame: Frame) -> None:
        ...
        extended_loop = (
            node.recursive or "loop" in find_undeclared(
                node.iter_child_nodes(
                    only=(
                        "body",)), ("loop",)) or any(
                block.scoped for block in node.find_all(
                    nodes.Block)))
